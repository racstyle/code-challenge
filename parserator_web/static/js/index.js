...
function displayParsedAddr(parsedAddr) {
   ...
   for (let address_part in address_components) {
      // create new row
...
addressForm.addEventListener('submit', async event => {
   ...
   try {
      const res = await fetch(`api/parse?address=${parsedInput}`) // call the API
         .then((response) => { return response.json(); } )   // parse the fetched response into readable JSON and return it
